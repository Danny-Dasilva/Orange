...
import gstreamer
class Camera:
    def __init__(self, render_size, loop, inference_size=None):
        if inference_size:
            self._layout = gstreamer.make_layout(render_size, inference_size)
        else:
...
class DeviceCamera(Camera):
    ...
    def make_pipeline(self, fmt, profile, inline_headers, bitrate, intra_period):
        if fmt == 'h264':
            ...
            return pipelines.cv2_streaming_pipeline(self._fmt, profile, bitrate, self._layout)
        else:
            return pipelines.camera_streaming_pipeline(self._fmt, profile, bitrate, self._layout)


def make_camera(source, inference_size=None, loop=None):
